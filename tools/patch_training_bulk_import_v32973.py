...
bundle = Path(
    'build_sources/v3.29.73-training-import/'
    'training_import_sources.tar.xz.b64'
)
...
# Extrai apenas os arquivos novos. O pacote original usa package:excel,
# porém abaixo substituímos somente o leitor XLSX para reaproveitar archive
# que já existe no Auditar SST, sem adicionar dependência ao app.
...
with tarfile.open(fileobj=io.BytesIO(raw), mode='r:xz') as archive:
    ...
    if found != allowed:
        raise RuntimeError(
            'Pacote do importador incompleto: ' + repr(sorted(found))
        )
    archive.extractall(root, members=members)

servicep = root / 'lib/services/training_import_service.dart'
service = servicep.read_text(encoding='utf-8')
...
xlsx_start = service.find('  static _ParsedDocument _parseXlsx(')
xlsx_end = service.find('  static _ParsedDocument _parseCsv(', xlsx_start)
if xlsx_start < 0 or xlsx_end < 0:
    raise RuntimeError('Bloco _parseXlsx não localizado.')

xlsx_impl = r"""  static _ParsedDocument _parseXlsx(
    Uint8List bytes,
    List<Worker> workers,
  ) {
    try {
      ...
      for (final file in archive.files) {
        ...
        if (name == 'xl/sharedStrings.xml') {
          sharedXml = utf8.decode(content, allowMalformed: true);
        } else if (
            RegExp(r'^xl/worksheets/sheet\d+\.xml$').hasMatch(name)) {
          sheets.add(utf8.decode(content, allowMalformed: true));
        }
      }
  ...
  static List<String> _xlsxSharedStrings(String xml) {
    ...
    final result = <String>[];
    final itemExpression =
        RegExp(r'<si(?:\s[^>]*)?>([\s\S]*?)</si>');
    final textExpression =
        RegExp(r'<t(?:\s[^>]*)?>([\s\S]*?)</t>');
  ...
  static List<List<String>> _xlsxSheetRows(
    ...
    final rowExpression =
        RegExp(r'<row(?:\s[^>]*)?>([\s\S]*?)</row>');
    final cellExpression =
        RegExp(r'<c\s+([^>]*)>([\s\S]*?)</c>');
...
service = service[:xlsx_start] + xlsx_impl + service[xlsx_end:]
servicep.write_text(service, encoding='utf-8', newline='\n')

# Entrada do importador no módulo Treinamentos da empresa.
if "import 'training_import_screen.dart';" not in train:
    marker = "import 'training_records_screen.dart';\n"
    if marker not in train:
        raise RuntimeError(
            'Import de training_records_screen não localizado.'
        )
    train = train.replace(
        marker,
...
screen = (
    root / 'lib/screens/training_import_screen.dart'
).read_text(encoding='utf-8')
test = (
    root / 'test/training_import_service_test.dart'
).read_text(encoding='utf-8')
...
assert "package:excel/excel.dart" not in service
