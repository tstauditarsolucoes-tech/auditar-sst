...
Future<Map<String, dynamic>> post(
  ...
  final r = await AppsScriptHttp.postJson(
    ...
    body,
    timeout: timeout,
    allowLongAndroidRequest: timeout.inSeconds > 30,
  );
...
Future<void> main() async {
  ...
  final pull = await post({
    ...
    'syncKey': key,
    'deviceId': 'v32960-final-readonly',
    'platform': 'android',
    'sinceVersion': 0,
    'limit': 1,
    ...
    try {
      final ai = await post({
        ...
        'payload': {
          ...
          'area': 'Ronda Expressa',
          'question': 'Descreva objetivamente apenas o que é visível na imagem de teste.',
      ...
      if (code == 'AI_PHOTO_INVALID_ARGUMENT') {
        throw StateError('A Central ainda recusou o formato JPEG normalizado: $ai');
      }
    } catch (e) {
      sw.stop();
      lastError = e;
