...
function authValidateToken_(token) {
  try {
    ...
    const payload = JSON.parse(Utilities.newBlob(Utilities.base64DecodeWebSafe(payloadEncoded)).getDataAsString('UTF-8'));
...
function authCreateToken_(user) {
  ...
  const payload = {
    ...
    role: user.role,
    iat: now,
    exp: now + EPI_AUTH_TOKEN_HOURS * 60 * 60 * 1000,
...
function authPublicUser_(user) {
  return {
    ...
    role: authNormalizeRole_(user.role),
