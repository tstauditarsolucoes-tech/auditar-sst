(() => {
  ...
  async function getEngine(){
    ...
    loading=(async()=>{
      ...
      engine=new window.Human.Human({backend:'webgl',modelBasePath:MODEL_BASE,cacheSensitivity:0.01,filter:{enabled:true,equalization:true},face:{enabled:true,detector:{rotation:true,return:true,maxDetected:2,minConfidence:0.52},mesh:{enabled:true},description:{enabled:true},iris:{enabled:false},emotion:{enabled:false},antispoof:{enabled:true},liveness:{enabled:true}},body:{enabled:false},hand:{enabled:false},object:{enabled:false},gesture:{enabled:false}});
  ...
  function setupOverlay(){
    ...
    const d=document.createElement('div');d.id='liveOverlay';d.className='live-overlay';d.innerHTML=`
      <div class="live-head"><div><h2>Verificação facial</h2><p>Olhe normalmente para a câmera. Não precisa piscar nem fazer gesto.</p></div><button id="liveClose" type="button" class="live-close">×</button></div>
      <div class="live-stage"><video id="liveVideo" autoplay muted playsinline></video><div class="live-frame"></div><div class="live-instruction" id="liveInstruction">Posicione o rosto dentro da marca<small>Fique olhando normalmente para a câmera</small></div></div>
      <div class="live-steps"><div id="liveFaceStep" class="live-step wait">1. Rosto</div><div id="liveRealStep" class="live-step wait">2. Pessoa real</div><div id="liveStableStep" class="live-step wait">3. Confirmando</div></div>
      <div class="live-note">A verificação é automática. Não é necessário piscar, sorrir ou virar a cabeça.</div>`;
  ...
  async function startLive(){
    ...
    styles();setupOverlay();goodFrames=0;lastReal=0;lastLive=0;running=true;liveWorkerId=workerId;
    $('#liveOverlay').classList.add('open');setStep('#liveFaceStep',false);setStep('#liveRealStep',false);setStep('#liveStableStep',false);
    ...
    try{
      ...
      $('#liveInstruction').innerHTML='Olhe normalmente para a câmera<small>Não precisa piscar nem fazer gesto</small>';
  ...
  async function loop(){
    ...
    try{
      ...
      if(!exactlyOne){goodFrames=0;setStep('#liveRealStep',false);setStep('#liveStableStep',false);$('#liveInstruction').innerHTML=faces.length>1?'Deixe somente uma pessoa na câmera<small>Olhe normalmente para a câmera</small>':'Posicione o rosto dentro da marca<small>Olhe normalmente para a câmera</small>';return schedule();}
      ...
      if(!realOk){$('#liveInstruction').innerHTML='Mantenha o rosto visível e bem iluminado<small>A verificação é automática</small>';goodFrames=0;setStep('#liveStableStep',false);return schedule();}
      goodFrames++;setStep('#liveStableStep',goodFrames>=2);
      $('#liveInstruction').innerHTML=goodFrames>=2?'Rosto validado ✓<small>Comparando com o cadastro biométrico…</small>':'Continue olhando normalmente<small>Quase pronto…</small>';
      if(goodFrames>=2)return passLive();
    }catch(_){goodFrames=0;setStep('#liveStableStep',false);}
    schedule();
  }
  ...
  async function passLive(){
    ...
    try{
      const file=new File([blob],'verificacao-facial.jpg',{type:'image/jpeg',lastModified:Date.now()});const dt=new DataTransfer();dt.items.add(file);const input=$('#bioCamera');input.files=dt.files;input.dispatchEvent(new Event('change',{bubbles:true}));
      watchVerification();
    }catch(err){toast('Falha ao encaminhar a verificação para a biometria.');clearPending();}
  }

  function watchVerification(){
    let tries=0;const timer=setInterval(()=>{tries++;const s=$('#bioVerifyStatus');if(s?.classList.contains('ok')){clearInterval(timer);s.insertAdjacentHTML('beforeend','<span class="bio-liveness-badge">✓ Pessoa real verificada automaticamente • sem piscar</span>');}else if(tries>80)clearInterval(timer);},100);
  }
  ...
  function bind(){
    const btn=$('#bioVerifyNow');if(btn&&!btn.dataset.liveBound){btn.dataset.liveBound='1';btn.textContent='🙂 Verificar rosto';btn.addEventListener('click',()=>setTimeout(startLive,0));}
