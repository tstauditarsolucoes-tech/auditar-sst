(() => {
  ...
  let currentUser=null,currentTenant=null,ready=false,observerTimer=null;
  ...
  function deviceLabel(){return `Campo Android • ${navigator.platform||'Dispositivo'}`.slice(0,80);}
  ...
  function tenantStoreKey(kind,id){return `gestaoEpiTenantStoreV2:${kind}:${id}`;}
  ...
  function prepareTenantStorage(tenantId){
    ...
    // Primeira associação após atualização: NÃO duplica, serializa nem apaga a base existente.
    // Isso evita travamento do WebView logo depois do login em bases com muitas assinaturas.
    if(!old){
      localStorage.setItem(LOCAL_TENANT_KEY,tenantId);
      return;
    }

    // Troca real de empresa: guarda a base bruta do tenant anterior e restaura a do novo.
    // É um caminho raro e evita misturar dados entre empresas.
    try{
      const oldApp=localStorage.getItem(APP_KEY)||'';
      const oldStock=localStorage.getItem(STOCK_KEY)||'';
      if(oldApp)localStorage.setItem(tenantStoreKey('app',old),oldApp);
      if(oldStock)localStorage.setItem(tenantStoreKey('stock',old),oldStock);
    }catch(_){}

    const nextApp=localStorage.getItem(tenantStoreKey('app',tenantId));
    const nextStock=localStorage.getItem(tenantStoreKey('stock',tenantId));
    if(nextApp)localStorage.setItem(APP_KEY,nextApp);else localStorage.removeItem(APP_KEY);
    if(nextStock)localStorage.setItem(STOCK_KEY,nextStock);else localStorage.removeItem(STOCK_KEY);
  ...
  function saveSession(t,u,tenant){
    ...
    localStorage.setItem(TENANT_CODE_KEY,tenant?.code||localStorage.getItem(TENANT_CODE_KEY)||'');
    localStorage.setItem(VALIDATED_AT_KEY,String(Date.now()));
    currentUser=u;currentTenant=tenant;
  }
  function clearSession(){localStorage.removeItem(TOKEN_KEY);localStorage.removeItem(USER_KEY);localStorage.removeItem(TENANT_KEY);localStorage.removeItem(VALIDATED_AT_KEY);currentUser=null;currentTenant=null;ready=false;}
  ...
  function injectOverlay(){
    ...
    const d=document.createElement('div');d.id='epiAuthOverlay';d.className='epi-auth-overlay';d.innerHTML=`<div class="epi-auth-card"><div class="epi-auth-mark">🦺</div><h1>Entrar no Gestão EPI</h1><p>Acesso exclusivo da empresa licenciada.</p><label>Código da empresa<input id="epiTenantCode" autocomplete="organization" autocapitalize="characters" placeholder="Ex.: EMPRESA-X"></label><label>Usuário<input id="epiAuthUser" autocomplete="username" autocapitalize="none" placeholder="Seu usuário"></label><label>Senha<input id="epiAuthPass" type="password" autocomplete="current-password" placeholder="Sua senha"></label><button id="epiAuthSubmit" type="button">Entrar</button><div id="epiAuthError" class="epi-auth-error"></div></div>`;
    document.body.appendChild(d);
    $('#epiTenantCode').value=localStorage.getItem(TENANT_CODE_KEY)||'';
  ...
  function fireReady(user,tenant){
    // Libera a tela primeiro; só depois avisa os outros módulos.
    requestAnimationFrame(()=>setTimeout(()=>document.dispatchEvent(new CustomEvent('gestao-epi-auth-ready',{detail:{user,tenant}})),250));
  }
  ...
  async function submitAuth(){
    ...
    if(!navigator.onLine){err.textContent='O login precisa de internet.';return;}
    ...
    try{
      ...
      hideLogin();
      requestAnimationFrame(()=>applyUser(r.user,r.tenant));
      ready=true;
      fireReady(r.user,r.tenant);
  ...
  function injectUserChip(){
    ...
    if(currentUser)chip.innerHTML=`<span><span class="epi-tenant-name">🏢 ${escapeHtml(currentTenant?.name||currentTenant?.code||'Empresa')}</span>👤 ${escapeHtml(currentUser.name||currentUser.username)}<br><small>${roleLabel(currentUser.role)}</small></span><button id="epiLogout" type="button">Sair</button>`;
  ...
  function applyRestrictions(){
    ...
    const admin=currentUser.role==='admin',campo=currentUser.role==='campo';
  ...
  function applyUser(u,tenant){
    currentUser=u;currentTenant=tenant||currentTenant;document.body.dataset.epiRole=u.role;resetHidden();injectUserChip();applyRestrictions();
    if(!$('#btnSaveDelivery')?.dataset.authStampBound){const b=$('#btnSaveDelivery');if(b){b.dataset.authStampBound='1';b.addEventListener('click',()=>setTimeout(stampLatestDelivery,220));}}
    if(!window.__gestaoEpiAuthObserver){
      window.__gestaoEpiAuthObserver=new MutationObserver(()=>{clearTimeout(observerTimer);observerTimer=setTimeout(applyRestrictions,250);});
  ...
  function stampLatestDelivery(){
    ...
    try{const root=JSON.parse(localStorage.getItem(APP_KEY)||'{}');const rows=Array.isArray(root.deliveries)?root.deliveries:[];const d=rows[0];if(!d||Date.now()-new Date(d.createdAt||0).getTime()>20000)return;d.tenantId=currentTenant.id;d.registeredBy={username:currentUser.username,name:currentUser.name||currentUser.username,role:currentUser.role};d.registeredByAt=new Date().toISOString();d.updatedAt=new Date().toISOString();localStorage.setItem(APP_KEY,JSON.stringify(root));document.dispatchEvent(new CustomEvent('auditar-epi-data-changed'));}catch(_){}
  }

  function logout(show=true,msg=''){clearSession();resetHidden();if(show||msg)showLogin(msg||'Você saiu do sistema.');}
  function useCachedOffline(){const u=savedUser(),t=savedTenant();if(!u||!t||!tokenStillValid()||!offlineSessionFresh())return false;currentUser=u;currentTenant=t;hideLogin();applyUser(u,t);ready=true;fireReady(u,t);toast('Modo offline • acesso temporário');return true;}
  ...
  async function init(){
    ...
    if(token()){
      try{
        const me=await api('tenant_me');
        if(me?.ok){saveSession(token(),me.user,me.tenant);hideLogin();requestAnimationFrame(()=>applyUser(me.user,me.tenant));ready=true;fireReady(me.user,me.tenant);return;}
        clearSession();
